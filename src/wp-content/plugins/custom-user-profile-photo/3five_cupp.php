...
function cupp_profile_img_fields( $user ) {
	...
	$avatar_id		 = get_the_author_meta( 'cupp_upload_meta', $user->ID );
	$upload_url      = wp_get_attachment_url($avatar_id);
				...
				<td>
					...
					<div id="cupp_upload">
						...
						<input class="hidden" type="hidden" name="cupp_upload_meta" id="cupp_upload_meta"
						       value="<?php echo $avatar_id ?>"/>
						<input class="hidden" type="hidden" name="cupp_upload_edit_meta" id="cupp_upload_edit_meta"
						       value="<?php echo '/wp-admin/post.php?post='.$avatar_id.'&action=edit&image-editor' ?>"/>
...
function cupp_save_img_meta( $user_id ) {
	...
	$values = array(
		...
		'cupp_upload_meta'      => filter_input( INPUT_POST, 'cupp_upload_meta', FILTER_VALIDATE_INT ),
...
//	$attachment_upload_url = esc_url( get_the_author_meta( 'cupp_upload_meta', $user_id ) );
    $attachment_upload_url = wp_get_attachment_url(get_the_author_meta( 'cupp_upload_meta', $user_id ));

	if ( $attachment_upload_url ) {
		// Grabs the id from the URL using the WordPress function attachment_url_to_postid @since 4.0.0.
		$attachment_id = get_the_author_meta( 'cupp_upload_meta', $user_id );
